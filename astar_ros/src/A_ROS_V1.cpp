...
// #include "/home/hitvarth/openCV/modules/core/include/opencv2/core.hpp"
...
void node::calc_gcost() // didn't get it
{
...

{
  if(!msg->data.empty())
  {
    ...
    nav_msgs::Path path;
    path.header.frame_id = "base_link"; //map


    nav_msgs::OccupancyGrid DilatedMap;
    DilatedMap.header.stamp = ros::Time::now();
    DilatedMap.header.frame_id = "occgrid"; // map
    DilatedMap.info.resolution = mapRes;
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber map_sub = n.subscribe<nav_msgs::OccupancyGrid>("/cv/laneoccgrid", 1, OccupancyGridCallback); 
